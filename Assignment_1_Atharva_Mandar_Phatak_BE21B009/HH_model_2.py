...
# Python dode to plot the graph which depicts the firing rate (frequency) in response to the applied external current 
...
#Function to find the peaks from the given range
...
#Iterating for external applied currents  
for ext_curr in tqdm(range((current_range.size))):
    hh_model(ext_curr)


#Plotting the graph with the threshold values of currents  
I1_var = 0.0224*np.ones(len(npeaks_hist))
I2_var = 0.0625*np.ones(len(npeaks_hist))
I3_var = 0.4498*np.ones(len(npeaks_hist))
